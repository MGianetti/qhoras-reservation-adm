...
function CalendarEventDialog({ refreshCalendar, roomsList }) {
    ...
    const safeBeginDate = eventBeginDate && !isNaN(new Date(eventBeginDate).getTime()) ? eventBeginDate : null;
    ...
    const initialValues = useMemo(
        () => ({
            roomTF:
                // if stateCalendar.room is set, use it,
                // otherwise default to the currently visible room (first in roomsList)
        ...
        [eventID, openDialog, room, roomsList]
    );

    const formik = useFormik({
        initialValues,
        enableReinitialize: true,
        validationSchema: baseValidationSchema(appointments, roomsList, eventID, calendarBlocks, safeBeginDate ? format(new Date(safeBeginDate), 'eeee', { locale: ptBR }) : ''),
        onSubmit: handleSubmit
    });

    const initialOptionTime = useMemo(() => {
        const weekday = safeBeginDate ? format(new Date(safeBeginDate), 'eeee', { locale: ptBR }) : 'monday';
    ...
    useEffect(() => {
        if (formik.values.beginDate && formik.values.beginTime && formik.values.endTime && initialOptionTime.length > 0) {
            let beginTimeValue = formik.values.beginTime.value;
            if (beginTimeValue.length === 4) {
                beginTimeValue = '0' + beginTimeValue;
            }
            const validBeginTime = initialOptionTime.find((time) => time.value === beginTimeValue);
            ...
            const validEndTime = initialOptionTime.find((time) => time.value === endTimeValue);
            let correctedEndTime = validEndTime || initialOptionTime[0];

            if (timeToMinutes(correctedEndTime.value) <= timeToMinutes(correctedBeginTime.value)) {
                const nextValidOption = initialOptionTime.find((time) => timeToMinutes(time.value) > timeToMinutes(correctedBeginTime.value));
    ...
    }, [formik.values.beginDate, formik.values.beginTime, formik.values.endTime, initialOptionTime]);
    ...
    const formatDateTime = (date, time) => {
        ...
        return dayjs(date).hour(Number(h)).minute(Number(m)).second(Number(s)).format('YYYY-MM-DD[T]HH:mm:ssZ');
    };
    ...
    return (
        <>
            ...
            <UpdateRecurrenceDialog scopeDialogOpen={scopeDialogOpen} setScopeDialogOpen={setScopeDialogOpen} doUpdate={doUpdate} />
        </>
    );
}
