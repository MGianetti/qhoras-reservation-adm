        ...
        >
          <Grid item xs={8}>
            <Typography className={classes.title}>
              {format(internalDate, "MMMM yyyy", { locale: ptBR })}
            </Typography>
          </Grid>
          <Grid
            item
          ...
          >
            {isDatepicker && (
              <Tooltip
                title={`${format(new Date(), "EEEE, d MMMM", { locale: ptBR })}`}
            ...
            return (
              <Grid item xs key={`small-calendar-column-header-${index}`}>
                <Typography className={classes.dayHeader}>
                  {format(weekDay, "eeee", { locale: ptBR }).substr(0, 1)}
                </Typography>
              </Grid>
            );
          })}
        ...
        {weeks.map((week, weekIndex) => (
          ...
          >
            {week.map((day, dayIndex) => {
              ...
              const isSelected =
                format(day, "ddMMyyyy", { locale: ptBR }) ===
                format(selectedDate, "ddMMyyyy", { locale: ptBR });
