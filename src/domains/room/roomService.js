...
import { readItem, addItem, updateItem, deleteItem, setLoading, clear } from './roomSlice';
...
const { read, create, update, deleteRoom } = {
    read: async ({ businessId, page = 1, limit = 10, search = '', ...filters }) => {
        try {
            ...
            const response = await roomRepository.readAllRooms(businessId, page, limit, filters, search);
