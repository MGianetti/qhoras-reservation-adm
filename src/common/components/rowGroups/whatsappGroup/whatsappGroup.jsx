...
const WhatsappGroup = () => {
  ...
  useEffect(() => {
    const checkClientStatus = async () => {
      try {
        const response =
          await whatsappRepository.checkWhatsAppHeartbeat(userId);
  ...
  const handleInitializeClient = async () => {
    ...
    try {
      const response =
        await whatsappRepository.initializeWhatsAppClient(userId);
      ...
      <Grid container alignItems={"center"} justifyContent={"space-between"}>
        <Box sx={{ display: "flex", flexDirection: "column" }}>
          <Typography variant="subtitle1" sx={{ color: "#606062" }}>
            {qrCode ? (
              "Escaneie o QR Code abaixo para ativar seu WhatsApp!"
            ) : isClientActive ? (
          ...
          <Button
            ...
            sx={{ mt: 3, mb: 2 }}
            onClick={
              isClientActive ? handleDeleteClient : handleInitializeClient
            }
