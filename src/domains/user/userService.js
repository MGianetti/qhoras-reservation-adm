...
import {
  updatedUserConfigSuccess,
  updatedUserConfigFail,
...
const { read, updateUserConfig, updateCompany, readBusinessEmployees } = {
  read: async (businessId) => {
    try {
      ...
      const userConfigResponse =
        await userRepository.readUserConfig(businessId);

      dispatch(
        readItem({
          schedule: scheduleResponse,
          suggestionTime: userConfigResponse.suggestionTime,
        }),
      );
    } catch (error) {
      return false;
  ...
  updateUserConfig: async (businessId, updateUserConfigPayload) => {
    try {
      dispatch(setLoading(true));
      const { schedule, userConfig } = await userRepository.updateUserConfig(
        businessId,
        updateUserConfigPayload,
      );
      dispatch(clearItems());
      dispatch(
        readItem({ schedule, suggestionTime: userConfig.suggestionTime }),
      );
  ...
  updateCompany: async (businessId, updateCompanyPayload) => {
    try {
      dispatch(setLoading(true));
      const response = await userRepository.updateCompany(
        businessId,
