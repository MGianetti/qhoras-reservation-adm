...
function Datepicker(props) {
    const { styleCls = null, withIcon = true, label = '', dateFormat = 'dd/MM/yyyy', originalValue = new Date(), onChange = () => {} } = props;
    ...
    const inputProps = {
        endAdornment: withIcon ? (
            <InputAdornment position="end">
                <IconButton size="medium" edge="end" aria-label="Toggle calendar visibility" onClick={handleOpenCalendar}>
    ...
    const dateValidation = (value) => {
        ...
        const validDateFormat = new RegExp(/^((\d{1,2})[-|.|/](\d{1,2})[-|.|/](\d{2,4}))|((\d{2,4})[-|.|/](\d{1,2})[-|.|/](\d{1,2}))$/);
    ...
    useEffect(() => {
        ...
        if (format(originalValue, 'yyyy/MM/dd', { locale: ptBR }) === '1970/01/01') {
            setOpenCalendar(false);
        }
