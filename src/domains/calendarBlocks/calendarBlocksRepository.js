...
const readCalendarBlocks = (userId, start, end) =>
  makeRequest(() =>
    apiService.get(
      endpoints.read.calendarBlocks.replace("${USER-ID}", userId),
      { params: { start, end } },
    ),
  );
const createCalendarBlock = (
  ...
  makeRequest(() =>
    apiService.post(
      endpoints.create.calendarBlock.replace("${USER-ID}", userId),
...
const deleteCalendarBlock = (blockId) =>
  makeRequest(() =>
    apiService.delete(
      endpoints.delete.calendarBlocks.replace("${BLOCK-ID}", blockId),
    ),
  );
