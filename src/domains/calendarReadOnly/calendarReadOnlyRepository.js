...
const readRoomsPublic = (businessId, page = 1, limit = 10, filters = {}) =>
    makeRequest(() =>
        apiService.get(endpoints.read.roomsPublic.replace('${BUSINESS-ID}', businessId), {
            params: { page, limit, filters }
        })
    );

const readCalendarPublic = (businessId, page, limit, order, orderBy) =>
    makeRequest(() => apiService.post(endpoints.read.calendarListPublic.replace('${BUSINESS-ID}', businessId), { page, limit, order, orderBy }));
