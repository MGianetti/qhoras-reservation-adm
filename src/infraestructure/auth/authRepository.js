...
const requestPasswordResetRepository = (email) => {
    return makeRequest(() => apiService.post(endpoints.auth.requestPasswordReset, { email }));
};

const sendNewPassword = ({ newPassword, token }) => {
    return makeRequest(() => apiService.post(endpoints.auth.resetPassword, { newPassword: newPassword, token: token }));
};
...
const refreshToken = (tokenToRefresh) => {
    return makeRequest(() => apiService.post(endpoints.auth.refreshToken, { token: tokenToRefresh }));
};
