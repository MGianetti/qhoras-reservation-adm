...
function CalendarBoard(props) {
    ...
    const getEventData = (day) => {
        ...
        const dayEvents = monthEvents.filter((event) => event.begin && format(new Date(event.begin), 'yyyyMMdd', { locale: ptBR }) === format(day, 'yyyyMMdd', { locale: ptBR }));
        ...
        const markers = eventsByHour.map((evHour) => {
            return dayEvents
                ...
                .map((event, index) => <EventMark key={`event-${event.id}`} calendarEvent={event} sq={index} len={evHour.len} />);
        });
    ...
    const getBlockData = (day) => {
        const dayBlocks = (calendarBlocks || []).filter((block) =>
            format(new Date(block.initialTime), 'yyyyMMdd', { locale: ptBR }) === format(day, 'yyyyMMdd', { locale: ptBR })
        );

        const sortedBlocks = dayBlocks.sort((a, b) => new Date(a.initialTime) - new Date(b.initialTime));
        const blockHoursEvents = sortedBlocks.map((block) => new Date(block.initialTime).getHours()).sort((numberA, numberB) => numberA - numberB);

        const eventsByHour = blockHoursEvents.reduce((acc, hour) => {
            const len = blockHoursEvents.filter((eventHour) => eventHour === hour).length;
            !acc.some((accItem) => accItem.hour === hour) && acc.push({ hour, len });
            return acc;
        }, []);
        ...
        const blockMarkers = eventsByHour.map((evHour) => {
            return sortedBlocks
                .filter((block) => new Date(block.initialTime).getHours() === evHour.hour)
                .map((block, index) => (
                    <EventMark
                        key={`block-${block.id}`}
                        calendarEvent={{
    ...
    const isAgendaOpen = (day) => {
        ...
        const currentBlock = scheduleState?.find((block) => block.day === translateDay[dayOfWeek]);

        if (!currentBlock || !currentBlock.isActive) return false;

        const [startHour, startMinutes] = currentBlock.startTime.split(':').map(Number);
        const [endHour, endMinutes] = currentBlock.endTime.split(':').map(Number);

        const initTime = dayjs(day).startOf('day').toDate();
        const startTime = dayjs(day).set('hour', startHour).set('minute', startMinutes);
        ...
        return viewLayout.map((index) => {
            ...
            return (
                ...
                >
                    ...
                    {((eventsOfDay && eventsOfDay.length > 0) || (blocksOfDay && blocksOfDay.length > 0)) && (
                    ...
                    {agendaOpenTime && (
                        <>
                            <div
                                style={{ left: 0, right: 0, height: `${agendaOpenTime.position}px`, width: '100%' }}
                                onClick={(e) => {
                                    e.stopPropagation();
                                }}
                            >
                            ...
                            <div
                                style={{ left: 0, right: 0, bottom: 0, top: `${agendaOpenTime.position + agendaOpenTime.duration}px` }}
                                onClick={(e) => {
                                    e.stopPropagation();
                                }}
                            >
    ...
    }, [classes, defaultEventDuration, getEventData, getBlockData, layout, selectedDate, selectedWeek, selectedWeekIndex, viewLayout, appointments, calendarBlocks]);

    return (
        <Grid ref={drop} onDrop={onDrop} container spacing={0} direction="row" justify="center" alignItems="flex-start" className={classes.board}>
            <LineDivisor />
