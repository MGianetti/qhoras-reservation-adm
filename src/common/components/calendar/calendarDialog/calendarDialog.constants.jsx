...
export const getServiceDuration = (serviceId, services) => {
...
export const getInitialAndEndTime = (userData, weekDay) => {
    ...
    const block = userData.filter((block) => block?.day === weekDay.toUpperCase());
    return {initialTime: block[0].startTime, endTime: block[0].endTime};
}

export const validationSchema = (appointments, servicesList, eventID, userData, calendarBlocks) =>
    Yup.object({
        serviceTF: Yup.string().required('Escolha um serviço.'),
        ...
        beginTime: Yup.object()
            ...
            .test('conflict', 'O horário selecionado conflita com outro agendamento.', function (value) {
                ...
                const initialBlock = userData.filter((block) => block.day === startDateWeek);
                const endBlock = userData.filter((block) => block.day === endDateWeek);

                if (initialBlock.length && endBlock.length) {
                    const blockStartTime = dayjs(`${formattedDate} ${initialBlock[0].startTime}`, 'YYYY-MM-DD HH:mm');
                    const blockEndTime = dayjs(`${formattedDate} ${initialBlock[0].endTime}`, 'YYYY-MM-DD HH:mm');

                    if (startDateTime.isBefore(blockStartTime) || endDateTime.isAfter(blockEndTime)) {
                        return this.createError({
                            message: 'O horário selecionado está fora do horário de funcionamento permitido.'
                        });
                    }
                }
                ...
                for (let block of calendarBlocksFiltered) {
                    ...
                    if (
                        (startDateTime.isBefore(blockEnd) && endDateTime.isAfter(blockStart)) || // Conflito total ou parcial
                        (startDateTime.isSame(blockStart) || endDateTime.isSame(blockEnd)) // Coincidência exata
                    ) {
                        return this.createError({
