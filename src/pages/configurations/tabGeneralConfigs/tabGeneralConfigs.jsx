...
function TabGeneralConfigs() {
  ...
  const {
    ...
    suggestionTime: suggestionTimeState,
    isLoading,
  ...
  const initialCheckedDays = days.reduce((acc, day, index) => {
    ...
    const userDataDay = scheduleState?.find(
      (d) => d.day === diasDaSemana[index],
    );
  ...
  const initialTimeRanges = days.reduce((acc, day, index) => {
    ...
    acc[day] = userDataDay
      ? [
          dayjs(`2022-04-17T${userDataDay.startTime}`),
          dayjs(`2022-04-17T${userDataDay.endTime}`),
        ]
  ...
  const handleSubmit = (values) => {
    const scheduleData = Object.entries(values.checkedDays).map(
      ([dia, isActive], index) => {
        const [startTime, endTime] = values.timeRanges[dia].map((time) =>
          time.format("HH:mm"),
        );
        return {
