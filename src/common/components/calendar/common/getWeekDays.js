import { startOfMonth, endOfMonth, startOfWeek, lastDayOfWeek, addDays, eachDayOfInterval } from 'date-fns';
...
function getWeekDays(selectedDate, size) {
    ...
    const weekly = (_month, _size) => _month.reduce((weeks, day, index, group) => (!(index % _size) ? weeks.concat([group.slice(index, index + _size)]) : weeks), []);
