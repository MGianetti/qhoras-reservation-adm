...
const { readRoom, readCalendarList, readAppointments } = {
  readRoom: async ({ businessId, page = 1, limit = 10, ...filters }) => {
    try {
      const response = await calendarReadOnlyRepository.readRoomsPublic(
        businessId,
        page,
        limit,
        filters,
  ...
  readCalendarList: async ({
    ...
    try {
      const response = await calendarReadOnlyRepository.readCalendarPublic(
        businessId,
        page,
        limit,
        order,
  ...
  readAppointments: async (roomId, start, end) => {
    ...
    try {
      const response = await calendarReadOnlyRepository.readAppointmentsPublic(
        roomId,
        start,
        end,
      );
